      ...
      <Container>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {featuredWallpapers.map((wallpaper, index) => (
            <div
              key={wallpaper.id}
              className="group relative aspect-[9/16] overflow-hidden rounded-[1.5rem] shadow-2xl transition-all duration-700 hover:-translate-y-2 hover:shadow-primary/20 animate-fade-in"
            ...
            >
              ...
              <div className="absolute inset-0 border border-white/5 rounded-[1.5rem] group-hover:border-white/20 transition-colors duration-700" />
            </div>
          ))}
        </div>
      </Container>
